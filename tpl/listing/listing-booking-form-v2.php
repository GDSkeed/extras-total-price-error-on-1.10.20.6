...
<div class="hfy-listing-booking-form">
    ...
    <div class="booking-price-block body">
                        ...
                        <div class="col">
                            <div class="form-group">
                                <div class="input-group">
                                    <input type="text" id="hotel-datepicker" placeholder="<?= esc_attr__('Check In', 'hostifybooking') . ' – ' . esc_attr__('Check Out', 'hostifybooking') ?>" readonly class="form-control hotel-datepicker" value="<?= esc_attr($dates_value) ?>"/>
                                </div>
                            </div>
                        </div>
                    <?php // V2 ?>
        ...
        <?php // PAYMENT SCHEDULE INFORMATION ?>
        
        <?php if (isset($paymentSchedule) && !empty($paymentSchedule->description)): ?>
...
<?php
$opts = get_option('hostifybooking-plugin');
if ($settings->direct_inquiry_email ?? false) {
    if (defined('HFY_USE_NEW_INQUIRY_FORM') && HFY_USE_NEW_INQUIRY_FORM) {
        include hfy_tpl('element/direct-inquiry-modal-v2');
    } else {
