...
var HotelDatepicker = (function (fecha) {
    ...
    class HotelDatepicker {
      constructor(input, options) {
        ...
        this.defaultMinNights = this.minNights; // Store the original default
        ...
        this.mobileMonthsToShow = opts.mobileMonthsToShow || 6;
        this.mobileMonthsToLoad = opts.mobileMonthsToLoad || 12;
        this.i18n = opts.i18n || {
          selected: "Your stay:",
          night: "Night",
          nights: "Nights",
          ...
          // Note: These default English values are overridden by WordPress translations in the main datepicker initialization
        ...
        this.calendarCustomStay = opts.calendarCustomStay || []; // Add new option
      ...
      init() {
        ...
        // Show months - handle differently for mobile
        if (this.isMobile) {
          this.showMonthsForMobile(defaultTime);
        } else {
      ...
      createDom() {
        ...
        // Create month dropdown for mobile if needed
        if (this.isMobile) {
      ...
      checkIsMobile() {
        return window.innerWidth < 768 || this.isTouchDevice() && window.innerWidth < 1024;
      }

      // Create a dropdown for quick month navigation on mobile
      createMonthDropdown() {
        const monthsContainer = this.datepicker.querySelector('.' + this.className + '__months');
        const dropdown = document.createElement('select');
        dropdown.className = this.className + '__month-dropdown';
        dropdown.setAttribute('aria-label', 'Select month');

        // Get current date and create options for the next 12 months
        const currentDate = new Date();
        for (let i = 0; i < this.mobileMonthsToLoad; i++) {
          const monthDate = new Date(currentDate);
          monthDate.setMonth(currentDate.getMonth() + i);
          const option = document.createElement('option');
          option.value = i;
          option.textContent = this.getMonthName(monthDate.getMonth()) + ' ' + monthDate.getFullYear();
          dropdown.appendChild(option);
        }

        // Add event listener to handle month selection
        dropdown.addEventListener('change', e => {
          const selectedIndex = parseInt(e.target.value, 10);
          this.jumpToMonth(selectedIndex);
        });

        // Insert dropdown at the top of the months container
        monthsContainer.insertBefore(dropdown, monthsContainer.firstChild);
      }

      // Jump to a specific month (for dropdown navigation)
      jumpToMonth(monthIndex) {
        const currentDate = new Date();
        const targetDate = new Date(currentDate);
        targetDate.setMonth(currentDate.getMonth() + monthIndex);

        // Scroll to the selected month
        const monthElements = this.datepicker.querySelectorAll('.' + this.className + '__month');
        if (monthElements.length > monthIndex) {
          ...
          // Log for debugging
          console.log(`Jumping to month index ${monthIndex}: ${this.getMonthName(targetDate.getMonth())} ${targetDate.getFullYear()}`);
        } else {
          console.warn(`Cannot jump to month index ${monthIndex}, only ${monthElements.length} months available`);
        }
      }

      // Show multiple months for mobile view
      showMonthsForMobile(defaultTime) {
        console.log("Showing months for mobile, starting with:", defaultTime);

        // Clear any existing months first
        const monthsContainer = this.datepicker.querySelector('.' + this.className + '__months');
        const existingMonths = monthsContainer.querySelectorAll('table');
        existingMonths.forEach(month => {
          if (month.parentNode === monthsContainer) {
            monthsContainer.removeChild(month);
          }
        });

        // FORCE starting from the current month regardless of selection
        const today = new Date();

        // Create an array to store all the months we'll display
        const monthsToShow = [];

        // Generate consecutive months starting from current month
        for (let i = 0; i < this.mobileMonthsToLoad; i++) {
          const newDate = new Date(today.getFullYear(), today.getMonth() + i, 1);
          monthsToShow.push(newDate);
        }

        // Log the months we're about to display
        monthsToShow.forEach((date, index) => {
          console.log(`Month ${index + 1}: ${date.getFullYear()}-${date.getMonth() + 1}`);
        });

        // Create and show each month
        monthsToShow.forEach((monthDate, i) => {
          const monthIndex = i + 1;

          // Create a unique ID for each month
          const monthId = 'month-' + monthIndex + '-' + this.generateId();

          // Create table element
          const table = document.createElement('table');
          table.setAttribute('role', 'presentation');
          table.setAttribute('id', monthId);
          table.className = this.className + '__month ' + this.className + '__month--month' + monthIndex;

          // Add header with month name
          const thead = document.createElement('thead');
          const headerRow = document.createElement('tr');
          headerRow.className = this.className + '__month-caption';

          // Create header with prev/next buttons for consistency
          const prevButtonCell = document.createElement('th');
          prevButtonCell.innerHTML = '<span role="button" tabindex="0" aria-label="' + this.i18n["aria-prev-month"] + '" class="' + this.className + '__month-button ' + this.className + '__month-button--prev" month="' + monthIndex + '">&lt;</span>';
          const headerCell = document.createElement('th');
          headerCell.colSpan = 5;
          headerCell.className = this.className + '__month-name';
          headerCell.textContent = this.getMonthName(monthDate.getMonth()) + ' ' + monthDate.getFullYear();
          const nextButtonCell = document.createElement('th');
          nextButtonCell.innerHTML = '<span role="button" tabindex="0" aria-label="' + this.i18n["aria-next-month"] + '" class="' + this.className + '__month-button ' + this.className + '__month-button--next" month="' + monthIndex + '">&gt;</span>';
          ...
          weekDaysRow.className = this.className + '__week-days';
          weekDaysRow.setAttribute('aria-hidden', 'true');
          weekDaysRow.setAttribute('role', 'presentation');
          weekDaysRow.innerHTML = this.getWeekDayNames(monthIndex);
          thead.appendChild(weekDaysRow);
          table.appendChild(thead);

          // Add tbody for the days
          const tbody = document.createElement('tbody');
          table.appendChild(tbody);

          // Add the table to the container
          monthsContainer.appendChild(table);

          // Show the month in the table - create a new date to avoid reference issues
          const monthDateCopy = new Date(monthDate.getTime());
          this.showMonth(monthDateCopy, monthIndex);
        });

        // Store the first and last months - but use the original defaultTime for selection
        this.month1 = new Date(defaultTime);
        this.month2 = new Date(defaultTime);
        this.month2.setMonth(this.month2.getMonth() + 1);

        // Scroll to the month containing the selection if there is one
        if (this.start) {
          setTimeout(() => {
            ...
            // Find the month element that contains the selection
            const monthElements = this.datepicker.querySelectorAll('.' + this.className + '__month');
            for (let i = 0; i < monthElements.length; i++) {
              const monthName = monthElements[i].querySelector('.' + this.className + '__month-name');
              if (monthName) {
                const monthText = monthName.textContent;
                if (monthText.includes(this.getMonthName(startMonth)) && monthText.includes(startYear.toString())) {
                  // Scroll to this month with a slight offset
                  monthElements[i].scrollIntoView({
                    behavior: 'smooth',
                    block: 'center'
                  });
                  console.log(`Scrolling to selected month: ${monthText}`);
                  break;
                }
              }
            }
      ...
      createDatepickerDomString() {
        ...
        // For mobile, we'll create months dynamically in showMonthsForMobile
        // For desktop, create the standard 2 months
        if (!this.isMobile) {
          // Print single months
          for (let i = 1; i <= 2; i++) {
      ...
      openDatepicker() {
        ...
        if (!this.isOpen) {
          ...
          // For mobile, always show months in chronological order starting from current month
          if (this.isMobile) {
            // Store the current selection
            const currentStart = this.start;
            const currentEnd = this.end;

            // Always show months starting from current month
            this.showMonthsForMobile(new Date());
          ...
          if (!this.inline) {
            // Use setTimeout to ensure dimensions are calculated after potential style changes
            setTimeout(() => {
              try {
                ...
                // Temporarily show datepicker to measure, but keep it invisible
                this.datepicker.style.visibility = 'hidden';
                this.datepicker.style.display = ''; // Ensure it's block or table for measurement
                this.datepicker.style.left = ''; // Reset any previous inline style

                const datepickerRect = this.datepicker.getBoundingClientRect();
                const inputRect = this.input.getBoundingClientRect();
                const parentRect = this.parent.getBoundingClientRect(); // Get parent bounds
                const viewportWidth = window.innerWidth;
                const safetyMargin = 10; // Small margin

                let newLeftStyle = ''; // Default: Let CSS handle it (usually aligns left edge of datepicker with left edge of input relative to parent)

                if (datepickerRect.width > 0) {
                  // Calculate default right edge position (relative to viewport)
                  // Assumes default CSS aligns left edge of datepicker with left edge of input
                  const defaultRightEdge = inputRect.left + datepickerRect.width;

                  // Calculate alternative left position (align right edges, relative to viewport)
                  const alternativeLeftPosViewport = inputRect.right - datepickerRect.width;
                  const overflowsRight = defaultRightEdge > viewportWidth - safetyMargin;
                  const hasSpaceOnLeft = alternativeLeftPosViewport > safetyMargin;

                  // console.log(`Input Left: ${inputRect.left}, Input Right: ${inputRect.right}`);
                  // console.log(`Picker Width: ${datepickerRect.width}`);
                  // console.log(`Default Right Edge: ${defaultRightEdge}, Viewport Width: ${viewportWidth}`);
                  // console.log(`Overflows Right: ${overflowsRight}`);
                  // console.log(`Alternative Left Pos (Viewport): ${alternativeLeftPosViewport}`);
                  // console.log(`Has Space on Left: ${hasSpaceOnLeft}`);

                  if (overflowsRight && hasSpaceOnLeft) {
                    // Calculate left position relative to the PARENT to align right edges
                    const newLeftRelativeToParent = inputRect.right - parentRect.left - datepickerRect.width;
                    newLeftStyle = newLeftRelativeToParent + 'px';
                    // console.log('Decision: Position Left. Style:', newLeftStyle);
                  } else {
                    // Default to CSS positioning (clear inline style)
                    newLeftStyle = '';
                    // console.log('Decision: Position Right (CSS Default). Style:', newLeftStyle);
                  }
                } else {
                  // console.warn('Datepicker dimensions not available.');
                }

                // Apply calculated position
                this.datepicker.style.left = newLeftStyle;
                // Ensure top is set correctly relative to parent (usually handled by CSS, but good to be explicit if needed)
                // this.datepicker.style.top = (inputRect.bottom - parentRect.top) + 'px'; // Example if needed

                // Make visible again and hide before animation
                this.datepicker.style.visibility = '';
                this.datepicker.style.display = 'none';
              } catch (e) {
                console.error("Error during datepicker repositioning:", e);
                this.datepicker.style.left = ''; // Fallback
                this.datepicker.style.visibility = '';
                this.datepicker.style.display = 'none';
              }

              // Now, trigger the slide animation after positioning logic
              this.slideDown(this.datepicker, this.animationSpeed);
            }, 0); // setTimeout 0 allows repaint/reflow
          }
          // --- End Dynamic Repositioning Logic ---

          // Slide down animation is now handled inside the setTimeout for non-inline
          // } // This closing brace seems misplaced from the original code structure, ensure it matches the `if (!this.isOpen)` block start
          ...
          // For mobile, scroll to the selected month if there is one
      ...
      checkAndSetDefaultValue() {
        ...
        if (dates && dates.length >= 2) {
          ...
          // Parse the dates
          const date1 = this.parseDate(dates[0], _format);
          const date2 = this.parseDate(dates[1], _format);

          // Store the selected dates
          this.start = date1.getTime();
          this.end = date2.getTime();

          // For mobile, we don't want to change the month display order
          // We'll just show the months in chronological order and scroll to the selected month
          if (this.isMobile) {
            // Show months for mobile - always starting from current month
            this.showMonthsForMobile(new Date());
            ...
            // For desktop, use the normal date range setting
            this.changed = false;
            this.setDateRange(date1, date2, onresize);
            this.changed = true;
          }
      ...
      dayClicked(day) {
        ...
        if (isSelectStart) {
          ...
          // --- Dynamic Min Nights Logic ---
          const startDate = new Date(time);
          let requiredMinStay = this.defaultMinNights; // Start with default

          if (startDate && Array.isArray(this.calendarCustomStay)) {
            for (let i = 0; i < this.calendarCustomStay.length; i++) {
              const rule = this.calendarCustomStay[i];
              if (rule && rule.date_start && rule.min_stay && typeof rule.date_start === 'string') {
                try {
                  const ruleStartDate = fecha__namespace.parse(rule.date_start, 'YYYY-MM-DD');
                  let match = false;
                  if (rule.date_end && typeof rule.date_end === 'string') {
                    const ruleEndDate = fecha__namespace.parse(rule.date_end, 'YYYY-MM-DD');
                    if (ruleStartDate && ruleEndDate && startDate >= ruleStartDate && startDate <= ruleEndDate) {
                      match = true;
                    }
                  } else {
                    if (ruleStartDate && startDate.getFullYear() === ruleStartDate.getFullYear() && startDate.getMonth() === ruleStartDate.getMonth() && startDate.getDate() === ruleStartDate.getDate()) {
                      match = true;
                    }
                  }
                  if (match) {
                    const customMinStay = parseInt(rule.min_stay, 10);
                    if (!isNaN(customMinStay) && customMinStay > 0) {
                      requiredMinStay = customMinStay;
                      break; // Apply first matching rule
                    }
                  }
                } catch (e) {
                  console.warn('Error parsing custom stay rule date:', rule, e);
                }
              }
            }
          }
          this.minNights = requiredMinStay;
          this.minDays = this.minNights > 1 ? this.minNights + 1 : 2;
          // --- End Dynamic Min Nights Logic ---
        } else if (this.start) {
          this.end = time;
          // Reset minNights to default when second date is selected
          this.minNights = this.defaultMinNights;
          this.minDays = this.minNights > 1 ? this.minNights + 1 : 2;
        }
      ...
      goToNextMonth(e) {
        ...
        // For mobile view, we don't need to do anything as all months are already displayed
        if (this.isMobile) {
          return true;
        }
      ...
      clearSelection() {
        ...
        this.start = false;
        // Reset minNights to default on clear
        this.minNights = this.defaultMinNights;
        this.minDays = this.minNights > 1 ? this.minNights + 1 : 2;
        this.end = false;
      ...
      isTouchDevice() {
        ...
        return "ontouchstart" in window || window.DocumentTouch && document instanceof DocumentTouch || navigator.maxTouchPoints > 0 || window.navigator.msMaxTouchPoints > 0;
      }
